...
app = FastAPI(
    title="RespireX API - Simple Version",
    description="Simple API for respiratory disease classification",
    version="1.0.0"
)
...
@app.get("/")
async def root():
    return {"message": "RespireX API - Simple Version", "status": "running"}
...
async def predict_disease(file: UploadFile = File(...)):
    ...
    try:
        # Just return a random prediction for demo
...
async def predict_from_annotation(request: AnnotationRequest):
    """Simple annotation-based prediction endpoint"""
    try:
        # Simple logic based on events

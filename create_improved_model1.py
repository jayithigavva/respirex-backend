"""
Improved Model 1 creation with better accuracy
...
def extract_improved_features(audio_path: str) -> np.ndarray:
    """Extract improved audio features for better accuracy."""
    try:
        ...
        features.extend([
            ...
            kurtosis(audio_norm),          # Kurtosis
            skew(audio_norm),              # Skewness
        ])
        
        # 2. MFCC features (simplified)
        mfccs = librosa.feature.mfcc(y=audio_norm, sr=sr, n_mfcc=13, hop_length=512)
        # Take mean and std of each MFCC coefficient
        for i in range(13):
            features.extend([
                np.mean(mfccs[i]),
                np.std(mfccs[i])
        ...
        return np.zeros(70)  # Return zero features if error

def create_improved_model1():
    """Create improved Model 1 with better accuracy."""
    logger.info("Creating improved Model 1 with better accuracy...")
    ...
    for idx, row in diagnosis_df.iterrows():
        ...
        if patient_id in patient_to_files:
            ...
            features = extract_improved_features(audio_path)
    ...
    selector = SelectKBest(score_func=f_classif, k=min(40, X_train_scaled.shape[1]))
    ...
    logger.info(f"Improved Model 1 Accuracy: {accuracy:.4f}")
    logger.info("Classification Report:")
    logger.info(classification_report(y_test, y_pred))
    
    # Save model, scaler, and selector
    os.makedirs('models', exist_ok=True)
    joblib.dump(best_model, 'models/model1_improved.joblib')
    joblib.dump(scaler, 'models/scaler1_improved.joblib')
    joblib.dump(selector, 'models/selector1_improved.joblib')
    
    logger.info("✅ Improved Model 1 saved successfully")
    
    return best_model, scaler, selector, accuracy

def main():
    """Create improved Model 1."""
    logger.info("Creating improved Model 1 with better accuracy...")
    
    try:
        model, scaler, selector, acc = create_improved_model1()
        logger.info(f"✅ Improved Model 1 created with accuracy: {acc:.4f}")
    except Exception as e:
        logger.error(f"❌ Failed to create Improved Model 1: {str(e)}")
    
    logger.info("🎉 Improved model creation completed!")
